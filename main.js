...
async function downloadAllImageFiles(plugin) {
    ...
    for (const file of fileArray) {
        // --- PATCH START: 防止 file.path 为 undefined/null 导致 startsWith 报错 ---
...
class Helper {
    ...
    getImageLink(value) {
        ...
        for (const match of matches) {
            ...
            // --- PATCH START: 确保路径和名称不为空 ---
            if (!path && !name) {
                continue;  // 跳过既没有路径也没有名称的匹配
            }
            // --- PATCH END ---
            fileArray.push({
                path: path || "",
        ...
        for (const match of WikiMatches) {
            // --- PATCH START: 防止 match[1] 为 undefined 导致 parse 报错 ---
            if (!match || !match[1]) {
                continue;  // 跳过无效的匹配
            }
            // --- PATCH END ---
...
class imageAutoUploadPlugin extends obsidian.Plugin {
    ...
    fileMenuUpload(file) {
        ...
        for (const match of fileArray) {
            // --- PATCH START: 防止 undefined/null 引起报错 ---
            if (!match || !match.path || typeof match.path !== "string") {
                continue;   // 跳过无法解析的图片引用
            }
            // --- PATCH END ---
            const imageName = match.name || "unknown";
        ...
        this.uploader.uploadFiles(imageList.map(item => item.path)).then(res => {
            if (res.success) {
                ...
                if (this.settings.deleteSource) {
                    imageList.map(image => {
                        // --- PATCH START: 防止 image.path 为 undefined/null 导致 startsWith 报错 ---
                        if (image && image.path && !image.path.startsWith("http")) {
    ...
    filterFile(fileArray) {
        const imageList = [];
        for (const match of fileArray) {
            // --- PATCH START: 防止 undefined/null 引起 startsWith 报错 ---
            if (!match || !match.path || typeof match.path !== "string") {
                continue;   // 跳过无法解析的图片引用（外链、空行、扫描错误等）
            }
            // --- PATCH END ---
    ...
    uploadAllFile() {
        ...
        for (const match of fileArray) {
            const imageName = match.name || "unknown";
            const encodedUri = match.path;
            if (encodedUri && encodedUri.startsWith("http")) {
                imageList.push({
                    path: match.path,
                    name: imageName,
                    source: match.source || "",
                ...
                // --- PATCH START: 防止 encodedUri 为 undefined/null 导致 decodeURI 报错 ---
                if (!encodedUri || typeof encodedUri !== "string") {
                    continue;   // 跳过无效的路径
                }
                // --- PATCH END ---
                const fileName = pathBrowserify.basename(decodeURI(encodedUri));
                ...
                if ((!file && decodedUri && decodedUri.startsWith("./")) ||
                    decodedUri && decodedUri.startsWith("../")) {
                    const filePath = pathBrowserify.resolve(pathBrowserify.join(basePath, pathBrowserify.dirname(activeFile.path)), decodedUri);
                    if (require$$0.existsSync(filePath)) {
                        const path = obsidian.normalizePath(pathBrowserify.relative(obsidian.normalizePath(basePath), obsidian.normalizePath(pathBrowserify.resolve(pathBrowserify.join(basePath, pathBrowserify.dirname(activeFile.path)), decodedUri))));
        ...
        this.registerEvent(this.app.workspace.on("editor-paste", (evt, editor, markdownView) => {
            ...
            if (this.settings.workOnNetWork) {
                ...
                const imageList = this.helper
                    ...
                    .filter(image => image && image.path && image.path.startsWith("http"))
                    .filter(image => image && image.path && !this.helper.hasBlackDomain(image.path, this.settings.newWorkBlackDomains));
                if (imageList.length !== 0) {
                    this.uploader
